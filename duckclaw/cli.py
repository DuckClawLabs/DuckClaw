...
def chat(model):
    ...
    try:
        ...
        _vision    = _cfg.llm.vision_model    or f"[dim] Add in settings [/dim]"
        _tts       = _cfg.llm.tts_model       or f"[dim] Add in settings [/dim]"
        console.print(
            f"[dim]  🧠 Reasoning model:    [/dim]{_reasoning}\n"
            f"[dim]  👁  Vision model:       [/dim]{_vision}\n"
            f"[dim]  🔊 Text-to-speech model:[/dim]{_tts}\n"
            f"[dim]  ℹ️  Update vision/TTS models + API keys at [bold]http://127.0.0.1:8741/settings[/bold][/dim]\n"
        )
    except Exception:
        pass

    # If --model was provided, ask for key and persist to .env
    if model:
        api_key = questionary.password(f"PRIMARY_MODEL_KEY for '{model}' (leave blank to keep existing):").ask() or ""
        if api_key:
            import re as _re
            _env_path = os.path.expanduser("~/.duckclaw/.env")
            _existing: dict = {}
            if os.path.exists(_env_path):
                for _line in open(_env_path).read().splitlines():
                    _m = _re.match(r"^([A-Z_]+)=(.*)$", _line.strip())
                    if _m:
                        _existing[_m.group(1)] = _m.group(2)
            _existing["PRIMARY_MODEL_KEY"] = api_key
            with open(_env_path, "w") as _f:
                _f.write("\n".join(f"{k}={v}" for k, v in _existing.items()) + "\n")
            os.chmod(_env_path, 0o600)
            console.print(f"[dim]  ✓ PRIMARY_MODEL_KEY saved to {_env_path}[/dim]\n")
...
llm:
  ...
  # Keys go in ~/.duckclaw/.env: PRIMARY_MODEL_KEY, REASONING_API_KEY, VISION_API_KEY, AUDIO_API_KEY
  reasoning_model: "{reasoning_model}"   # e.g. claude-sonnet-4-5-20251001, deepseek/deepseek-r1
  vision_model: "{vision_model}"         # e.g. gemini/gemini-2.0-flash
  tts_model: "{audio_model}"             # e.g. gemini/gemini-2.0-flash or groq/whisper-large-v3
  cost_tracking: true
  max_tokens: 4096
  temperature: 0.7
...
def _setup_browser():
    ...
    console.print("[dim]Answer a couple of questions, then we'll open the settings page in your browser.[/dim]\n")

    backend_choice = questionary.select(
        "Where do you want to run the AI?",
        choices=[
            questionary.Choice("Ollama    — local inference, no API key needed", value="ollama"),
            questionary.Choice("3rd party — Claude / Gemini / Groq (API key required)", value="cloud"),
        ]
    ).ask()
    ...
    if backend_choice == "ollama":
        model_name = questionary.text(
            "Model name:", default="ollama/gemma3:1b"
        ...
        model_name = questionary.select(
            "Choose model:",
            choices=[
                questionary.Choice("claude-haiku-4-5-20251001         — fast, affordable", value="claude-haiku-4-5-20251001"),
                questionary.Choice("claude-sonnet-4-6                  — powerful, balanced", value="claude-sonnet-4-6"),
                questionary.Choice("gemini/gemini-2.0-flash            — free tier", value="gemini/gemini-2.0-flash"),
                questionary.Choice("groq/llama-3.3-70b-versatile       — ultra-fast, free tier", value="groq/llama-3.3-70b-versatile"),
                questionary.Choice("groq/deepseek-r1-distill-llama-70b — reasoning, free tier", value="groq/deepseek-r1-distill-llama-70b"),
                questionary.Choice("Enter custom model string...", value="__custom__"),
            ]
        ).ask()
        ...
        if model_name == "__custom__":
            model_name = questionary.text("Model string:").ask() or "claude-haiku-4-5-20251001"
        api_key = questionary.password("PRIMARY_MODEL_KEY:").ask() or ""
        if api_key:
            env_entries["PRIMARY_MODEL_KEY"] = api_key
...
def _setup_cli_wizard():
    ...
    console.print("\n[bold cyan]Step 1/3 — Primary model[/bold cyan]")
    backend_choice = questionary.select(
        "Where do you want to run the AI?",
        choices=[
            questionary.Choice("Ollama    — local inference, no API key needed", value="ollama"),
    ...
    # ── Step 2: Specialist models (optional) ──────────────────────────────────
    console.print("\n[bold cyan]Step 2/3 — Specialist models[/bold cyan]")
    console.print("[dim]Leave blank to use your primary model for all tasks.[/dim]\n")

    reasoning_model = questionary.text(
        "Reasoning model (e.g. claude-sonnet-4-5-20251001, deepseek/deepseek-r1):",
        default="",
    ).ask() or ""
    if reasoning_model:
        rkey = questionary.password(f"{reasoning_model} REASONING_API_KEY (leave blank if same provider):").ask() or ""
        if rkey:
            env_entries["REASONING_API_KEY"] = rkey

    vision_model = questionary.text(
        "Vision model (e.g. gemini/gemini-2.0-flash, claude-haiku-4-5-20251001):",
        default="",
    ).ask() or ""
    if vision_model:
        vkey = questionary.password(f"{vision_model} VISION_API_KEY (leave blank if same provider):").ask() or ""
        if vkey:
            env_entries["VISION_API_KEY"] = vkey

    audio_model = questionary.text(
        "Audio/TTS model (e.g. gemini/gemini-2.0-flash, groq/whisper-large-v3):",
        default="",
    ).ask() or ""
    if audio_model:
        akey = questionary.password(f"{audio_model} AUDIO_API_KEY (leave blank if same provider):").ask() or ""
        if akey:
            env_entries["AUDIO_API_KEY"] = akey
    ...
    config_path = _write_config(
        config_dir, model_name, dashboard_port, enable_audit,
    ...
    if env_entries:
        ...
        with open(env_path, "w") as f:
            for k, v in env_entries.items():
                f.write(f"{k}={v}\n")
        os.chmod(env_path, 0o600)
        keys_saved = ", ".join(env_entries.keys())
        completion_lines += f"[green]✓[/green] Keys saved to [bold]{env_path}[/bold] [dim]({keys_saved})[/dim]\n"
